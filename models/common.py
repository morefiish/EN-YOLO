...
class MBConvBlock(nn.Module):
    ...
    def forward(self, x, drop_connect_rate=None):
        ...
        return x
# end EfficientNet主干网络
    
# BEGIN MobileNetV3 主干网络
class SeBlock(nn.Module):
    def __init__(self, in_channel, reduction=4):
        super().__init__()
        self.Squeeze = nn.AdaptiveAvgPool2d(1)
...
# END MobileNetV3 主干网络
        
# BEGIN ShuffleNetv2 主干网络
...
# END ShuffleNetv2 主干网络

# BEGIN coordattention 注意力机制
...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
# END coordattention 注意力机制
    
# SAFM block
class SAFM(nn.Module):
    def __init__(self, dim=320, n_levels=4):
...
# Begin SPPPCSPC_BiFPN
from torch.cuda.amp import autocast
from inspect import signature
from functools import partial
from typing import Dict, Tuple

class SPPCSPC_BiFPN(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(SPPCSPC_BiFPN, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
        ...
        self.cv6 = Conv(c_, c_, 3, 1)
        self.cv7 = Conv(2 * c_, c2, 1, 1)
        # self.bifpn = BiFPN_Add2(320, 640)
        self.bifpn = BiFPN_Add2(512, 1024)
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        # return self.cv7(torch.cat((y1, y2), dim=1))
        return self.cv7(self.bifpn([y1, y2]))
...
class Conv_Gelu(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_Gelu, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
